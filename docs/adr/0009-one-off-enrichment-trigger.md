...
**Add an explicit enrichment trigger — `enrichment.enrichOneOffs` mutation backed by `EnrichmentService.enrichOneOffs` — the analog of "close session".** The service method is a deep module: select pending one-off captures (`sessionId: null`, `entry: null`), oldest-first; Phase 1 (awaited) create `processing` placeholder entries so the UI flips to ping dots immediately; Phase 2 (fire-and-forget, concurrency 3) enrich each via the existing `enrichCapture` (reused unchanged), flipping entries to `pending_review`. Return `{ queuedCount }`. The router call is one line.
...
The two-phase trigger pattern (await placeholders → fire-and-forget enrich) now appears in exactly two places: `session.close` (`createPlaceholderEntries` + `enrichSessionCaptures`) and `enrichOneOffs`. Per ADR 0006, we extract at 3+ uses, not 2. The enrichment logic itself (`enrichCapture`) is already shared — only the thin trigger orchestration is duplicated, and that's the correct state to leave it in.