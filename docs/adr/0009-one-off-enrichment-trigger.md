...
The two-phase trigger pattern (await placeholders → fire-and-forget enrich) now appears in exactly two places: `session.close` (`createPlaceholderEntries` + `enrichSessionCaptures`) and `enrichOneOffs` (`createOneOffPlaceholderEntries` + `enrichOneOffCaptures`). Both follow the same convention: the router owns the detach. Per ADR 0006, we extract at 3+ uses, not 2.

The enrichment logic itself is **not** shared between the two paths. Session captures use `enrichSessionCaptures`, which builds a single `EnrichmentPipeline` with session-specific context (source, enrichment context, existing entries). One-off captures use `enrichOneOffCaptures`, which calls `enrichCapture` per capture — a standalone pipeline without session context. The two paths share `EnrichmentPipeline` at the lower level, but the per-path enrichment orchestration is genuinely different. Only the thin trigger pattern (await placeholders → fire-and-forget enrich) is duplicated, and that is the correct state to leave it in.