    ...
    <div style={{ fontFamily: "system-ui, sans-serif", padding: "2rem", maxWidth: 600, margin: "0 auto" }}>
      ...
      <form onSubmit={handleSubmit} style={{ display: "flex", gap: "0.5rem", marginBottom: "2rem" }}>
        ...
        <button
          ...
          style={{ padding: "0.5rem 1rem", fontSize: "1rem", borderRadius: 4, border: "1px solid #ccc", cursor: "pointer" }}
        >
      ...
      {captures.data && captures.data.length > 0 && (
        <ul style={{ listStyle: "none", padding: 0 }}>
          {captures.data.map((capture) => (
            <li key={capture.id} style={{ padding: "0.5rem 0", borderTop: "1px solid #eee" }}>
              <span style={{ fontSize: "1.1rem" }}>{formatCapture(capture)}</span>
              {capture.sourceHint && (
