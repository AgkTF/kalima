/**
 * PROTOTYPE — iteration 2. Refined variant B (compact + expand).
 * /prototype/review-cards?variant=b1|b2|b3
 *
 * Changes from B:
 * - Better expand toggle label
 * - Visual distinction between tags and related entries
 */
...
function Switcher({
  ...
  const labels: Record<string, string> = {
    b1: "Chevron toggle + labeled related",
    b2: "Icon toggle + comma related",
...
export function PrototypeReviewCards() {
  ...
  const Card = variant === "b2" ? CardB2 : variant === "b3" ? CardB3 : CardB1;
  ...
  return (
    <main className="flex min-h-dvh flex-col bg-page font-ui text-ink pb-20">
      ...
      <Switcher variants={["b1", "b2", "b3"]} current={variant} onSwitch={(v) => setSearchParams({ variant: v })} />
    </main>
  );
}
